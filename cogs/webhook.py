...
# =================================================================
# Kelas View untuk Tombol Interaktif di Pesan Final (WEBHOOK)
# =================================================================
...
class WebhookCog(commands.Cog):
    ...
    def load_single_role_messages(self):
        """Memuat ID pesan yang hanya boleh memilih 1 role."""
    ...
    def save_single_role_messages(self):
        """Menyimpan ID pesan yang hanya boleh memilih 1 role."""
        if not os.path.exists(self.data_dir):
            os.makedirs(self.data_dir)
        try:
            ...
            logging.error(f"Gagal menyimpan single role messages: {e}")
    ...
    async def set_single_role(self, ctx, message_id: str, channel: discord.TextChannel = None):
        """
        Mengatur pesan tertentu agar user hanya bisa memilih 1 role saja.
        Usage: !set_single_role <ID_PESAN> [#KANAL_PESAN]
        """
        try:
            await ctx.message.delete()
        except discord.errors.NotFound:
        ...
        try:
            ...
            await ctx.send("❌ Error: ID Pesan tidak valid. Pastikan Anda memasukkan ID numerik yang benar.", ephemeral=True)
            return

        try:
            # PENTING: Gunakan fetch_message pada target_channel untuk mengambil pesan lama
            message = await target_channel.fetch_message(message_int_id)
        except discord.errors.NotFound:
            await ctx.send(f"❌ Error: Pesan dengan ID `{message_id}` tidak ditemukan di {target_channel.mention}. Pastikan ID dan Channel sudah benar.", ephemeral=True)
            return
        except discord.errors.Forbidden:
            await ctx.send(f"❌ Error: Bot tidak memiliki izin (`Read Message History`) untuk melihat pesan di {target_channel.mention}.", ephemeral=True)
            return

        guild_id_str = str(ctx.guild.id)
        message_id_str = str(message.id)
        
        if guild_id_str not in self.single_role_messages:
            self.single_role_messages[guild_id_str] = []

        if message_id_str in self.single_role_messages[guild_id_str]:
            # Hapus batasan
            self.single_role_messages[guild_id_str].remove(message_id_str)
            self.save_single_role_messages()
            await ctx.send(f"✅ Batasan **single role** pada pesan `{message_id_str}` di {target_channel.mention} telah **dihapus**.", ephemeral=True)
        else:
            # Tambahkan batasan
            self.single_role_messages[guild_id_str].append(message_id_str)
            self.save_single_role_messages()
            await ctx.send(f"✅ Batasan **single role** pada pesan `{message_id_str}` di {target_channel.mention} telah **diaktifkan**.", ephemeral=True)
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if action == 'role':
            if not interaction.message:
                await interaction.response.send_message("Kesalahan: Interaksi tidak terkait dengan pesan.", ephemeral=True)
                return
            ...
            try:
                ...
                # 1. Cek jika role sudah dimiliki (sesuai permintaan: hanya ambil, tidak hapus jika sudah punya)
                if role_to_add in interaction.user.roles:
                    await interaction.response.send_message(f"Anda sudah memiliki role **{role_to_add.name}**.", ephemeral=True)
                    return

                # 2. LOGIKA SINGLE ROLE
                is_single_role = (
                    str(interaction.guild.id) in self.single_role_messages and
                    str(interaction.message.id) in self.single_role_messages[str(interaction.guild.id)]
                )
                
                roles_to_remove = []
                if is_single_role:
                    # Dapatkan semua ID role yang terkait dengan tombol di pesan ini
                    current_message_role_ids = set()
                    
                    # Ambil tombol-tombol dari pesan yang sama
                    for component in interaction.message.components:
                        for child in component.children:
                            if child.custom_id in self.button_actions:
                                action_data_btn = self.button_actions[child.custom_id]
                                if action_data_btn.get('action') == 'role':
                                    try:
                                        current_message_role_ids.add(int(action_data_btn.get('value')))
                                    except (ValueError, TypeError):
                                        continue

                    # Cari role yang harus dihapus
                    for role in interaction.user.roles:
                        if role.id in current_message_role_ids and role.id != role_to_add.id:
                            roles_to_remove.append(role)
                    
                    if roles_to_remove:
                        await interaction.user.remove_roles(*roles_to_remove)
                # 3. Berikan role yang dipilih

                await interaction.user.add_roles(role_to_add)
                
                notification = f"Anda telah mendapatkan role **{role_to_add.name}**!"
                if roles_to_remove:
                    notification += f" (Role lain dari pesan ini dihapus.)"

                await interaction.response.send_message(notification, ephemeral=True)
